...
clean_twitter <- function(english_tweets,cutoff=0,stopwords = stopwords::stopwords()){
  clean_text <- english_tweets$text %>% 
    str_replace_all("\\n"," ") %>% 
  ...
  users <- english_tweets$author_id
  
  #drop documents with 0 words
  zero_word_docs <- rowSums(dw_mat) == 0
  dw_mat <- dw_mat[!zero_word_docs,]
  users <- users[!zero_word_docs]
  original_ids <- (1:nrow(english_tweets))[!zero_word_docs]
  
  #du <- data.frame(doc_id = 1:nrow(english_tweets),user_id = english_tweets$author_id)
